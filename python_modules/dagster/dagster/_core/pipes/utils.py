...
from typing import IO, Dict, Iterator, Mapping, Optional, Sequence, Tuple, TypeVar, Union
...
class PipesBlobStoreMessageReader(PipesThreadedMessageReader):
    ...
    def __init__(
        self,
        interval: float = 10,
        log_readers: Optional[
            Union[Sequence["PipesLogReader"], Mapping[str, "PipesLogReader"]]
        ] = None,
    ):
        # convert old-style sequence to mapping
        if isinstance(log_readers, abc.Sequence):
            log_readers = {str(i): reader for i, reader in enumerate(log_readers)}

        log_readers = check.opt_mapping_param(
